...
void OCLCracker::Init()
{
    ...
    /*int i, x;
    uint data_in[HASH_SIZE * 128] = {0};
    uint results_out[HASH_SIZE * 32] = {0};
    int data_in_len = 128;
    int results_out_len = 32;
    int count = DATA_SIZE;
    int blocks_in_count = 2;

    //data_in = malloc(data_in_len * count);
    //results_out = malloc(results_out_len * counta);
    memset(data_in, 0x30, sizeof(data_in));
    memcpy(data_in, "lamepass", 8);
    ...
    */
    ...
    //
    context = clCreateContext(0, numEntries, device_id, &onOpenCLError, NULL, &err);
    if (!context)
    {
    ...
    //
    err = 0;
    err  = clSetKernelArg(kernel, 0, sizeof(unsigned int), &input);
    err |= clSetKernelArg(kernel, 1, sizeof(unsigned int), &output);
    err |= clSetKernelArg(kernel, 2, sizeof(unsigned int), &iterations);
    if (err != CL_SUCCESS)
    {
