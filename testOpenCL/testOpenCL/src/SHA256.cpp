...
uint SHA256::rotl(const uint x, const uint y)
{
    return ( x<<y | x>>(32-y) );
}

uint SHA256::rotr(const uint x, const uint y)
{
    return ( x>>y | x<<(32-y) );
}

uint SHA256::Ch(const uint x, const uint y, const uint z)
{
    return ( z ^ (x & ( y ^ z)) );
}

uint SHA256::Maj(const uint x, const uint y, const uint z)
{
    return ( (x & y) | (z & (x | y)) );
}

uint SHA256::S0(const uint x)
{
    return (rotr(x,2) ^ rotr(x,13) ^ rotr(x,22));
}

uint SHA256::S1(const uint x)
{
...
uint SHA256::bytereverse(const uint x)
{
    return ( ((x) << 24) | (((x) << 8) & 0x00ff0000) | (((x) >> 8) & 0x0000ff00) | ((x) >> 24) );
}

void SHA256::sha256_transform(uint *state, uint * data)
{
...
void SHA256::Finalize()
{
	...
	words = i >> 2;
	for (i = 0; i < words; i++) data[i] = (*((block + 4 * i)) << 24) | (*((block + 4 * i) + 1) << 16) | (*((block + 4 * i) + 2) << 8) | (*((block + 4 * i) + 3));
...
void SHA256::Digest(unsigned char * output)
{
	if (output != 0)
	{
		for (int i = 0; i < 8; i++)
